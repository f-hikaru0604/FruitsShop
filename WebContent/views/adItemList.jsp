...
<script type="text/javascript">
 ...
 //  テーブルソートの共通変数　設定要！
 //===============================================================
var gTabldID = 'itemTable';  // テーブルのエリアのIDを設定
...
%>
<table border="1" style="width:800px;" id="itemTable">
    <tr>
        <th>画像</th>
        <th cmanSortBtn>商品No</th>
        <th cmanSortBtn>商品名</th>
        <th>色</th>
        <th cmanSortBtn>価格（円）</th>
        <th>商品情報</th>
        <th cmanSortBtn>在庫数</th>
    </tr>
    <%
        for (ItemBean item : itemList) {
    %>
    <tr><td><img src="<%=item.getPng_path()%>" width="100" height="80"></td>
        <td><%=item.getItem_no()%></td>
        <td><%=item.getItem_name()%></td>
        <td><%=item.getItem_color()%></td>
        <td><%=item.getItem_price()%></td>
        <td><%=item.getItem_info() %></td>
        <td><%=item.getItem_count()%></td></tr>
